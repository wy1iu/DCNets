...
class VGG():
    ...
    def build(self, rgb, n_class, is_training):        
        ...
        #32X32        
        n_out = 96
        feat = self._conv_layer(feat, ksize, n_out, is_training, name='root', bn=True, 
            pad='SAME', norm=True, reg=False, orth=True)

        for i in range(n_layer):
            feat = self._resnet_unit_v1(feat, ksize, n_out, is_training, name='block1_unit'+str(i), stride=1, 
                norm=True, reg=False, orth=True)

        #16X16
        n_out = 192
        feat = self._resnet_unit_v1(feat, ksize, n_out, is_training, name='block2_unit0', stride=2, 
            norm=True, reg=False, orth=True)
        for i in range(1, n_layer):
            feat = self._resnet_unit_v1(feat, ksize, n_out, is_training, name='block2_unit'+str(i), stride=1, 
                norm=True, reg=False, orth=True)

        #8X8
        n_out = 384
        feat = self._resnet_unit_v1(feat, ksize, n_out, is_training, name='block3_unit0', stride=2, 
            norm=True, reg=False, orth=True)
        for i in range(1, n_layer):
            feat = self._resnet_unit_v1(feat, ksize, n_out, is_training, name='block3_unit'+str(i), stride=1, 
                norm=True, reg=False, orth=True)
        
        feat = tf.nn.avg_pool(feat, [1,8,8,1], [1,1,1,1], 'VALID')
        self.score = self._conv_layer(feat, 1, n_class, is_training, "score", bn=False, 
            pad='VALID', norm=False, reg=True, orth=False)
