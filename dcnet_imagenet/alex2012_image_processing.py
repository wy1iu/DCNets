...
def eval_image(image, height, width, scope=None):
  ...
  with tf.name_scope(values=[image, height, width], name=scope,
    ...
    distorted_image = tf.image.resize_images(image, [256, 256])
    distorted_image = tf.image.central_crop(distorted_image, 0.875)
    distorted_image.set_shape([height, width, 3])
    return distorted_image
...
def image_preprocessing(image_buffer, bbox, train, thread_id=0):
  ...
  imagenet_mean = tf.constant([121.6, 116.6, 102.7])
  imagenet_mean = tf.expand_dims(tf.expand_dims(imagenet_mean, 0), 1)
